...
async fn run_main_loop(
    ...
    loop {
        ...
        poll_pending_acks(tui_state, &mut pending_acks);
                ...
                match key.code {
                    ...
                    KeyCode::Char('c') | KeyCode::Char('C') => {
                        if tui_state.page == Page::Log {
                            ...
                            let market_to_close = match tui_state.page {
                                Page::Overview => tui_state
                                    ...
                                    .and_then(|market| {
                                        tui_state
                                            ...
                                            .positions
                                            .iter()
                                            .find(|position| position.market == market.symbol)
                                    })
                            ...
                            if let Some(market) = market_to_close {
                                ...
                                tui_state
                                    .set_banner("当前选中的市场没有持仓可平", BannerLevel::Warning);
                            } else {
                                tui_state.set_banner("当前没有可平仓持仓", BannerLevel::Warning);
                            }
                        }
                    }
                            ...
                            && tui_state.active_log_pane == LogPane::Commands
                        {
                            if let Some(command) = tui_state.selected_command() {
                                if command.cancellable {
                                    ...
                                    tui_state
                                        .set_banner("选中的命令当前不可取消", BannerLevel::Warning);
                                }
                            }
                        }
                    }
...
fn poll_pending_acks(tui_state: &mut TuiState, pending_acks: &mut Vec<PendingAck>) {
    let mut remaining = Vec::new();
    for mut pending in pending_acks.drain(..) {
        match pending.receiver.try_recv() {
            Ok(message) => handle_command_ack(tui_state, &pending.label, message),
