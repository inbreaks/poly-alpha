...
- `pulse_arb` 独立 runtime 已完成 `paper / live-mock` MVP 装配与验收覆盖
...
`pulse_arb` 是一条独立于旧 basis runtime 的新策略栈。

当前明确边界：

- 只做 `BTC / ETH`
- options anchor 只支持 `Deribit`
- hedge 只支持 `Binance USD-M perp`
- 运行模式只支持 `paper` 和 `live-mock`
- 不支持真钱 live trading
- 已接真实 `Deribit / Polymarket / Binance` feed-driven runtime 主循环
- `audit pulse-session-detail` 已能回放 session lifecycle，active session 期间的原始盘口 tape 也会落进审计事件
...
cargo run -p polyalpha-cli -- paper pulse inspect --env default
cargo run -p polyalpha-cli -- audit pulse-sessions --env default
cargo run -p polyalpha-cli -- audit pulse-session-detail --env default --pulse-session-id <session_id>
```

如果你只是做本地烟雾验证，也可以继续用已经验证过的：

```bash
cargo run -p polyalpha-cli -- paper pulse run --env live-ready-smoke --assets btc
cargo run -p polyalpha-cli -- live pulse run --env live-ready-smoke --assets btc --executor-mode mock
```
...
最近一轮 `2026-04-12` 的真实 `paper/live-mock` 验收结论：

- `BTC / ETH` 在 `paper` 和 `live-mock` 下都能稳定起真实 runtime、monitor socket 和 audit writer
- `Deribit anchor` 的 expiry 选择、local receive-time freshness、`Binance USD-M IOC limit hedge` 都已按 pulse runtime 口径接通
- live window 里的主要剩余门槛不再是“runtime 没链路”，而是 `poly_quote_stale` 和机会本身不够过线；真实样本已经能落出 `PulseSession`，但 sampled window 里仍未必能稳定落出非零 fill 的会话
