...
## 13. 异常排查必须先做反证，不接受“局部成立就当整体成立”

这是当前项目新增的硬规则，用来防止重复犯同一类错误。

默认要求：

- 只要某个 rejection reason、PnL 尖峰、连接异常、胜率异常出现频率异常，就不能只看单个测试或单个样本下结论。
- 必须先证明它是“末端正常护栏”，再允许把它归因为合理现象。
- 如果还没完成反证检查，就不能说 ready、合理、符合预期。

最小检查清单：

- 看 raw audit / event，确认异常到底发生在哪个 gate
- 看分布，不只看单个 case，要看它是不是系统性重复出现
- 看上游是否本该更早拦住，而不是让末端护栏背锅
- 看 sim / paper / live mock / parity reference 是否仍然是同一口径
...
- 单个测试是绿的，就默认整体没问题
- 某个护栏逻辑本身合理，就停止继续追上游根因
- 没看 raw event / rejection distribution，就直接解释现象

这条规则优先级很高。后续如果我没有按这套方式排查，你可以直接视为没有遵守项目记忆。
