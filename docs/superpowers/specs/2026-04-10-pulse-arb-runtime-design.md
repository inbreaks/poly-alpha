...
- `BTC` 在 `100k` 附近和 `ETH` 在 `2k` 附近，固定绝对 bump 对应的是完全不同的相对扰动
- 固定 bump 会让不同资产、不同价格区间下的 `event_delta` 灵敏度失真
- bump 过小会放大局部插值噪声、行情抖动和浮点误差
- bump 过大又会把关键 strike 附近的局部曲率抹平
...
- 先用参考价格 `S_ref` 计算相对 bump
- `dS = clamp(S_ref * delta_bump_ratio, min_abs_bump, max_abs_bump)`
- `S_ref` 应优先使用 anchor 对齐后的 `index_price`，而不是 perp 成交价

默认建议：

- `delta_bump_ratio` 初始可设为 `1 bp`，并允许按资产 override
- `min_abs_bump` 用来防止 bump 小到落入数值噪声区
- `max_abs_bump` 用来防止临近关键 strike 时过度平滑局部 Gamma

这条设计的目标不是让 `dS` 动态追着每一次波动乱跳，而是让不同资产在相近的相对尺度上计算 `event_delta`，同时维持数值稳定性。
...
如果两者持续偏离过大，说明当前局部链面或插值稳定性不足，应当把该时刻视为定价质量下降，而不是继续把不稳定的 `event_delta` 往下游对冲链路传播。
...
- 显式 event-delta bump 模式、relative ratio 和绝对 floor/cap
